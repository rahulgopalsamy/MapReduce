...
*The program answers questions related to time and year.
*/
public class Problem2Q8 {
		...
		if(line.split(delim).length==11){
			for (int i = 0; i <11;i++){
				tokens[i] = line.split(delim)[i];
                       ...
                       if(tokens[7].equals("Unknown")){return;}
                          else{
                          	time = tokens[7];
                                year = tokens[3];
				cap = Integer.parseInt(tokens[9]);
				word.set(year+"_"+time);
 ...
 if(max < val.get()){
  ...
  public static void main(String[] args) throws Exception {
    ...
    job.setJarByClass(Problem2Q8.class);
